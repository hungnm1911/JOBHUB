...
| Company Staff tenant resolution and business authorization (V3 F02, F14, F15) | `backend/src/services/company.service.js` (`resolveCompanyStaffTenant`, `assertCompanyStaffBusinessAccess`, `resolveCompanyStaffBusinessContext`, `resolveCompanyManagerRecruiterManagementContext`) | Approved middleware and future Company Staff business services | Parallel tenant derivation from client `companyId`/`managerUserId`, or reimplementing layered business-access gates outside the company service |
