...
`COMPLETED AND VERIFIED`. V10 has approved Product/Data contracts; Slices 01–03
(Application persistence foundation; V9 compatibility + Job-retention
compatibility; Unassigned Applications + Primary Application View) are
`IMPLEMENTED AND VERIFIED`. Subsequent V10 slices remain deferred by the
approved slice order. V11 through V17 must not be implemented merely from
