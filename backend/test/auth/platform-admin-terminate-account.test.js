...
const createApprovedActiveCompanyForManager = async ({
  managerUserId,
  reviewedByUserId,
  businessRegistrationNumber = "BRN-TERMINATE-BYPASS-1",
  name = "Terminate Bypass Co",
}) => {
...
describe("POST /api/platform-admin/accounts/:userId/terminate", () => {
  ...
  it("rejects account-level terminate of Company Manager owning APPROVED + ACTIVE Company", async () => {
    const agent = createTestAgent();

    const { user: admin } = await createVerifiedUser({
      email: "admin.terminate-bypass@example.com",
      role: USER_ROLE.PLATFORM_ADMIN,
    });
    ...
    const response = await agent
      .post(`/api/platform-admin/accounts/${manager._id.toString()}/terminate`)
      .set("Authorization", `Bearer ${adminAccessToken}`);
